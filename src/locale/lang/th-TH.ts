...
import { useBuddhistYear } from "../../composables/useBuddhistYear";
export default {
  name: 'ชื่อ',
  tel: 'โทรศัพท์',
  save: 'บันทึก',
  ...
  vanCalendar: {
    ...
    monthTitle: (year: number, month: number) => {
        ...
        const { toBuddhistYear } = useBuddhistYear();
        return toBuddhistYear(moment(`${year}-${month}-01`).locale('th'),'เดือน MMMM ปี YYYY');
    },
